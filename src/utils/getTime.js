...
export const helloInit = () => {
  ...
  ElMessage({
    ...
    message: `<strong>${hello}</strong> 歡迎來到我的小站`,
  });
};
...
export const siteDateStatistics = (startDate) => {
  ...
  if (differenceInYears >= 1) {
    return `本站誕生至今已有 ${Math.floor(differenceInYears)} 年 ${Math.floor(
      differenceInMonths % 12
    )} 月 ${Math.round(differenceInDays % 30)} 天`;
  } else if (differenceInMonths >= 1) {
    return `本站誕生至今已有 ${Math.floor(differenceInMonths)} 月 ${Math.round(
      differenceInDays % 30
    )} 天`;
  } else {
    return `本站誕生至今已有 ${Math.round(differenceInDays)} 天`;
  }
};
